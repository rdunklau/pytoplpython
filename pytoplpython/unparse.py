...
class CreateFunction(DDLElement):
    ...
    def __init__(self, name, return_type, args_definition, code):
        self.name = name
...
class Unparser:
    ...
    def __init__(self, tree, engine, file = sys.stdout):
    ...
    def _FunctionDef(self, t):
        self.write("\n")
        if 'postgresql_function' in [deco.id for deco in t.decorator_list]:
            body = StringIO()
            Unparser(t.body, self.engine, body)
            body.seek(0)
            create = CreateFunction(t.name, self._get_type(t.returns),
                    [(arg.arg, self._get_type(arg.annotation))
                        for arg in t.args.args], body.read())
            self.engine.execute(create)
            self.fill("def " + t.name + "(")
            self.dispatch(t.args)
            self.write(")")
            self.enter()
            self.fill("return func.%s(%s)" % (t.name, ','.join(arg.arg for arg in t.args.args)))
            self.leave()
    ...
    def _arg(self, t):
        self.write(t.arg + ' :' + t.annotation.id)
